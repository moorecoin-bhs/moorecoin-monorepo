...
async function loadLedger() {
  ...
  try {
    const response = await fetch(`${apiBase}/ledger/public?limit=50`);
    if (!response.ok)
    ...
    list.innerHTML = `<li class="ledger-empty">Couldn't load the ledger. Try refreshing.</li>`;
  } finally {
...
function renderLedger(entries) {
  ...
  list.innerHTML = entries
    .map((entry) => {
      ...
      const from = entry.fromPublicId ?? "—";
      const to = entry.toPublicId ?? "—";
